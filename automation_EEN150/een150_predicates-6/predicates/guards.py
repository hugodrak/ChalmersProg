...
class Eq:
    ...
    def eval(self, state: State) -> bool:
        ...
        if state.contains(self.value_or_variable):
            return state.get(self.variable) == state.get(self.value_or_variable)
        else:
            return state.get(self.variable) == self.value_or_variable
...
class And:
    ...
    def eval(self, state) -> bool:
        ...
        for i in range(1, len(self.guards)):
            if self.guards[i-1].eval(state) != self.guards[i].eval(state):
                all_equal = False
        return all_equal
