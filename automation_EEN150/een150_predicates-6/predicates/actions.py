...
class Assign(object):
    ...
    def next(self, state: State) -> State:
        ...
        """
        if state.contains(self.value_or_variable): # If variable
...
class Inc(object):
    ...
    def next(self, state: State) -> State:
        ...
        """
        try:
            state_var_value = state.get(self.variable)
        except:
            raise NextException(f"{self.variable} not in state!")
        if type(state_var_value) != int:
            raise NextException(f"State variable: {self.variable}, with value: {state_var_value} is not an int") 

        if type(self.inc) != int:
            raise NextException(f"Incrementation: {self.inc} is not an int") 


        return state.next(**{self.variable: state_var_value + self.inc})
...
class Dec(object):
    ...
    def next(self, state: State) -> State:
        ...
        if type(self.dec) != int:
            raise NextException(f"Incrementation: {self.dec} is not an int") 


        return state.next(**{self.variable: state_var_value - self.dec})
...
class Next(object):
    ...
    def next(self, state: State) -> State:
        ...
        if state_var_value not in self.domain:
            raise NextException(f"{state_var_value} not in domain")

        index_in_domain = self.domain.index(state_var_value)
        next_index = index_in_domain + 1 if index_in_domain < len(self.domain)-1 else 0
        return state.next(**{self.variable: self.domain[next_index]})
